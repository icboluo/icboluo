    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (null != userList && !userList.isEmpty()) {
                ...
                request.getRequestDispatcher("/z2/add.jsp").forward(request, response);
                return;
            }
            ...
            //响应数据
            if (addFlag) {
                response.sendRedirect(Constant.context_path + "/queryAllServlet");
            } else {
                request.setAttribute("errorMsg", "新增数据失败");
                request.getRequestDispatcher("/z2/error.jsp").forward(request, response);
            }
        } catch (Exception e) {
            request.setAttribute("errorMsg", "新增数据严重失败 " + e.getMessage());
            request.getRequestDispatcher("/z2/error.jsp").forward(request, response);
        }
    }
}
