...
class N1791_2497_寻找图的中心点 {
    public static void main(String[] args) {
        ...
        System.out.println(cla.maxStarSum(new int[]{1, 2, 3, 4, 10, -10, -20}, new int[][]{{0, 1}, {1, 2}, {1, 3}, {3, 4}, {3, 5}, {3, 6}}, 2));
        // -1
        System.out.println(cla.maxStarSum(new int[]{-2, -1, -2}, new int[][]{{0, 2}}, 1));
    }
    ...
    private int topK(LinkedList<Integer> neighbors, int val, int k, int[] vals) {
        ...
        if (!pq.isEmpty()) {
            // 也可以在构建图的时候直接不要这个边
            return pq.stream().mapToInt(Integer::intValue).filter(a -> a > 0).sum() + val;
        }
        return val;
    }
}
