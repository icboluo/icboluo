...
//        这串代码（like 语句匹配表达式以通配符开始）不会走索引，会进行全表扫描，效率极低
        System.out.println("select * from article where title like '%mysql%';");
        System.out.println("复合索引（先查第一个，第一个相同再查第二个）在查询中只对第一个有效果，");
        System.out.println("or 两边都需要索引，有一个没有索引就会导致全表扫描");
        System.out.println("状态值、性别等等，字段值较少，建立索引也大概率没有什么效果，mysql认为索引不如全表扫描");
