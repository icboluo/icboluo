...
public class NoteServiceImpl implements NoteService {
     ...
     */
    @Override
    public NoteAllVO selectOne(TimeNoteQuery query) {
        ...
        NoteAllVO noteVO = Stream.of(noteList1, noteList2, noteList3, noteList4)
                .flatMap(Collection::stream)
                .map(a -> BeanHelper.copyProperties(a, NoteAllVO::new))
                .min(Comparator.comparing(NoteAllVO::getGmtModified))
                .orElse(new NoteAllVO());
    ...
    public Map<String, String> selectIdAndType(TimeNoteQuery query) {
        NoteAllVO noteVO = selectOne(query);
    ...
    @Override
    public void updateFinishTime(DDD dd) {
        String type = dd.getType();
        Integer id = dd.getId();
        if (Constant.TIME_TYPE.equals(type)) {
