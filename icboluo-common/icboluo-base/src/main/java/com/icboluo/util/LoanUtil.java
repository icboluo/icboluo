...
public class LoanUtil {
     ...
     * @return 已经过的月期数（从1开始）
     */
    public static int monthPeriod(LocalDate startDate) {
    ...
    public static int remainPeriod(LocalDate startDate, int totalPeriods) {
        return totalPeriods - monthPeriod(startDate);
    }

    /**
     * 计算等额本息月供
     * <p>公式：月供 = 本金 × 月利率 × (1+r)^n ÷ ((1+r)^n - 1)
     *
    ...
    /**
     * 计算一次性提前还款后剩余期数（利用对数公式）
     *
     * @param monthlyPayment     原月供
     * @param remainingPrincipal 提前还款后剩余本金
     * @param monthlyRate        月利率
     * @return 剩余还款期数
     */
     ...
     * <p>公式：新月供 = 剩余本金 × 月利率 × (1+r)^n ÷ ((1+r)^n - 1)
     *
     * @param remainingPrincipal 提前还款后的剩余本金
     * @param monthlyRate        月利率
     * @param remainingPeriods   剩余还款期数（不变）
     * @return 新的月供金额
     */
    public static BigDecimal calculateReducedMonthlyPayment(BigDecimal remainingPrincipal, BigDecimal monthlyRate, int remainingPeriods) {
        BigDecimal compoundFactor = BigDecimal.ONE.add(monthlyRate).pow(remainingPeriods);
    ...
    /**
     * 计算提前还款减少月供后的总利息
     * <p>从提前还款后的剩余本金开始，按新月供逐月计算
     *
     * @param remainingPrincipal 提前还款后的剩余本金
     * @param monthlyRate        月利率
     * @param remainingPeriods   剩余还款期数
     * @return 减少月供后的总利息
     */
     ...
     * @return 本月利息
     */
    public static BigDecimal calculateMonthlyInterest(BigDecimal remainingPrincipal, BigDecimal monthlyRate) {
