    ...
    final void putMapEntries(Map<? extends K, ? extends V> m, boolean evict) {
        ...
        if (s > 0) {
            if (table == null) { // pre-size
                ...
                while (s > threshold && table.length < maxCapacity)
                    resize();
            }
...
//            就在数组上
            if (firstEle.hash == newHash &&
                ...
                cur = firstEle;
            } else if (firstEle instanceof TreeNode tempTree) {
                cur = tempTree.putTreeVal(this, arr, newHash, key, value);
            } else {
                int count = 0;
                Node<K, V> pre = firstEle;
                while (true) {
                    cur = pre.next;
                    if (cur == null) {
                        pre.next = newNode(newHash, key, value, null);
                        if (count >= treeifyThreshold - 1) {
                            treeifyBin(arr, newHash);
                        }
                        break;
                    }
                    if (cur.hash == newHash &&
                            ((k = cur.key) == key || (key != null && key.equals(k)))) {
                        break;
                    }
                    pre = cur;
                    count++;
...
// 旧都为0
    ...
    static final class TreeNode<K, V> extends Entry<K, V> {
        ...
        TreeNode<K, V> putTreeVal(MyHashMap8<K, V> map, Node<K, V>[] tab,
                                  int h, K k, V v) {
            Class<?> kc = null;
